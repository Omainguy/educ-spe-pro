...
import { BookOpen, Search, Filter, Plus, ExternalLink, Bookmark } from 'lucide-react';
import { MOCK_KNOWLEDGE } from '../constants';

const KnowledgePage: React.FC = () => {
  const [entries, setEntries] = useState(MOCK_KNOWLEDGE);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-1 space-y-6">
              ...
              {['Tout', 'Législation', 'Pratique', 'Institutionnel', 'Note Personnelle'].map((cat, i) => (
                <button 
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                    i === 0 ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-slate-500 hover:bg-slate-50'
                  }`}
                >
                  {cat}
                </button>
          ...
          <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-5 rounded-2xl text-white shadow-lg shadow-indigo-100">
            ...
            <h4 className="font-bold mb-1">Astuce IA</h4>
