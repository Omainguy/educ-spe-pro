...
import { Search, Plus, MoreHorizontal, Layout, Copy } from 'lucide-react';
            ...
            <div className="flex items-center gap-4 flex-wrap">
                ...
                <div className="flex gap-2">
                    {['Tous', 'Rapports', 'Notes', 'Projets'].map((cat, i) => (
                        <button
                            key={cat}
                            className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${i === 0 ? 'bg-indigo-600 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
                                }`}
                        >
                            {cat}
                        </button>
